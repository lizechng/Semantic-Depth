...
class Zhoushan_preprocessing(object):
    def __init__(self, dataset_path, input_type='depth'):
        self.train_paths = {'img': [], 'depth_in': [], 'velo_in': [], 'lidar_gt': [], 'seg_gt': []}
        self.val_paths = {'img': [], 'depth_in': [], 'velo_in': [], 'lidar_gt': [], 'seg_gt': []}
    ...
    def get_paths(self):
        ...
        for type_set in os.listdir(self.dataset_path):
            for root, dirs, files in os.walk(os.path.join(self.dataset_path, type_set)):
                self.train_paths['depth_in'].extend(sorted([os.path.join(root, file) for file in files
                                                            if re.search('depth', root)
                                                            and re.search('train', root)
                                                            and re.search('png', file)]))
                self.val_paths['depth_in'].extend(sorted([os.path.join(root, file) for file in files
                                                          if re.search('depth', root)
                                                          and re.search('val', root)
                                                          and re.search('png', file)]))
                self.train_paths['velo_in'].extend(sorted([os.path.join(root, file) for file in files
                                                           if re.search('velo', root)
                                                           and re.search('train', root)
                                                           and re.search('png', file)]))
                self.val_paths['velo_in'].extend(sorted([os.path.join(root, file) for file in files
                                                         if re.search('velo', root)
                                                         and re.search('val', root)
                                                         and re.search('png', file)]))
                self.train_paths['lidar_gt'].extend(sorted([os.path.join(root, file) for file in files
                                                            if re.search('lidar', root)
                                                            and re.search('train', root)
                                                            and re.search('png', file)]))
                self.val_paths['lidar_gt'].extend(sorted([os.path.join(root, file) for file in files
                                                          if re.search('lidar', root)
                                                          and re.search('val', root)
                                                          and re.search('png', file)]))
                self.train_paths['seg_gt'].extend(sorted([os.path.join(root, file) for file in files
    ...
    def prepare_dataset(self):
        ...
        if self.use_rgb:
            ...
            print(len(self.train_paths['depth_in']))
            print(len(self.train_paths['velo_in']))
            print(len(self.train_paths['img']))
            print(len(self.train_paths['lidar_gt']))
            print(len(self.train_paths['seg_gt']))
            print(len(self.val_paths['depth_in']))
            print(len(self.val_paths['velo_in']))
            print(len(self.val_paths['img']))
            print(len(self.val_paths['lidar_gt']))
            print(len(self.val_paths['seg_gt']))
    ...
    def compute_mean_std(self):
        ...
        stds = np.array([])
        max_lst = np.array([])
        for i, raw_img_path in tqdm.tqdm(enumerate(self.train_paths['lidar_gt'])):
            raw_img = Image.open(raw_img_path)
            raw_np = depth_read(raw_img, 0)
            vec = raw_np[raw_np >= 0]
            # vec = vec/84.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', default='./Zhoushan')
    parser.add_argument('--dest', default='/usr/data/tmp/')
    args = parser.parse_args()

    dataset = Zhoushan_preprocessing(args.datapath, input_type='rgb')
