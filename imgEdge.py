...
# Paths settings
parser.add_argument('--save_path', default='imgEdge/', help='save path')
...
class SmoothEdgeLoss(torch.nn.Module):
    ...
    def forward(self, depthPred, img, segGt):
        ...
        edgeX = torch.max(torch.zeros_like(grad_depth_x), self.beta - grad_depth_x) * grad_img_x
        edgeY = torch.max(torch.zeros_like(grad_depth_y), self.beta - grad_depth_y) * grad_img_y
